...
export default Service.extend({
    ...
    pushMessage: function (options) {
        let notification = notifObj.create({
            ...
            closeAction: options.isCloseable ? options.closeAction : null,
            isPersistent: options.isPersistent,
            actions: options.actions,
            progress: options.progress
        });
        this.messages.pushObject(notification);
        if (!notification.get('isPersistent')) {
            later(this, () => {
    ...
    progress(options) {
        options.type = 'info';
        options.isPersistent = isPresent(options.isPersistent) ? options.isPersistent : true;
